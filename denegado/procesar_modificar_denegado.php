...
ComenzarTransaccion($db);
			try {
				$db->Execute("update PLA_AUDITORIA.denegado
								set novedad=?,
								descripcion=?,
								fecha_cedula=to_date(?,'dd/mm/yyyy'),
								vto=?
							WHERE 	id_denegado=?", array($novedad,$apenom,$fechacedula,$vto,$idid));				
...
try {
 ...
 $rs_uu = $db ->Execute("SELECT us.descripcion as uu FROM 
	 ...
	 $describa='Siendo las '.$serhora.' horas,  El Agente '.$auditado.' MODIFICA Cedulas UIF en  fecha '.$serfecha;
...
//$fhasta=$_POST['fhasta'];
//die();
header ("location:adm_denegado.php?fecha=$fecha&idid=$idid&fecha_cedula=$fecha_cedula&fecha_vto=$fecha_vto");
?>