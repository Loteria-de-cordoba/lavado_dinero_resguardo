...
//empty($_POST['primedig']) || !is_numeric($_POST['primedig']) || || (empty($_POST['ultdig']) and $ultimo<>0) || (!is_numeric($ultimo) and $ultimo<>0) 
{
?>
...
ComenzarTransaccion($db);	
						try {
						$rs_controlcuit=$db->Execute("select lavado_dinero.check_CUIT(lavado_dinero.md5(?)) as controcuit from dual",
...
ComenzarTransaccion($db);	
						try {
						$db->Execute("insert into lavado_dinero.DENEGADO(
																   DESCRIPCION,
																   CUIT,
																   DOCUMENTO,
																   FECHA_ALTA,
							  ...
							  values (?,lavado_dinero.MD5(?),lavado_dinero.MD5(?),to_date(?,'DD/MM/YYYY'),?,?,?,to_date(?,'DD/MM/YYYY'),?)",
							  array($apenom,
									$cuit,
									 substr($docu,0,8),
									 $fecha,
