...
{
//EJERZO AUDITORIA
	 $describa='Siendo las '.$serhora.' horas,  El Agente '.$auditado.' consulta Cedulas U.I.F. Eliminadas';
...
try {
		$rs_total = $db ->Execute("select count(*) as total
									from lavado_dinero.denegado_eliminado b
									WHERE 1=1
									$condicion_idid
									$condicion_fecha_cedula
									$condicion_fechavto");
...
<tr>
	<td colspan="12" height="50" align="center" valign="bottom" class="texto4" scope="col">Administracion de Cedulas U.I.F. Eliminadas</td>
</tr>
<tr>
<td colspan="12">&nbsp;
</td>
	  ...
	  //obtengo cedulas alertadas por vencer
			try {
					$rs_alert = $db ->Execute("select count(*) as cuenta
							  from lavado_dinero.denegado x
							  where  sysdate between add_months(x.fecha_cedula-9,6) and add_months(x.fecha_cedula-1,6)
							  and x.id_denegado=?
							  and x.vto='S'",array($row->IDID));
						}									catch (exception $e){die ($db->ErrorMsg());} 
