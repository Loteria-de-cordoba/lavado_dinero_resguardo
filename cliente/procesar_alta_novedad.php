...
//die();
			...
			//echo $apostador;
			//die();
			if(isset($_POST['textapostador'.$i]) and substr(strtolower($_POST['textapostador'.$i]),0,5)<>'**reg')
			{
...
//{
			...
			//if($textapostador<>'' or $apostador==0)
				if($textapostador<>'' and $apostador==0)
					...
					//echo "paso ".$monto."apost".$textapostador;
						...
						try {
					$rs_cl = $db->Execute("select PLA_AUDITORIA.SEQ_T_CLIENTE.nextval  as clisec from dual");
					}
					catch  (exception $e) 
					{ 
					die($db->ErrorMsg());
					...
					$secuencia_cli = $row_cl->CLISEC;
						...
						$db->Execute("insert into PLA_AUDITORIA.t_CLIENTE(
							  ...
							  array(strtoupper($textapostador),
									 $casino,
									 $fecha,
									 'DU'.$_SESSION['usuario'],
									 $secuencia_cli));
	...
	if($apostador<>0 and $apostador<>-1)
	{
				ComenzarTransaccion($db);			
			try {
				$rs = $db->Execute("select PLA_AUDITORIA.SEQ_ID_NOVEDAD_CASINO.nextval as secuencia from dual");
				}
				catch  (exception $e) 
				{ 
				die($db->ErrorMsg());
			...
			//die();
			try {
				$db->Execute("insert into PLA_AUDITORIA.t_novedad_CASINO (
