...
$habilitado_valoracion=0;//habilita la posibilidad de valorar
$habilita_apertura=0;//habilita para abrir jornadas solo administradores
while ($i<$_SESSION['cantidadroles'])  {
	...
	if (($_SESSION['rol'.$i]=='LAVADO_DINERO_CONF_DELE' and $ccuenta<>0)|| ($_SESSION['rol'.$i]=='ROL_LAVADO_DINERO_OP_UNICO' and $ccuenta<>0) || ($_SESSION['rol'.$i]=='ROL_LAVADO_DINERO_CASINO_CARGA' and $ccuenta<>0) || ($_SESSION['rol'.$i]=='ROL_LAVADO_DINERO_ADMINISTRA')  || ($_SESSION['rol'.$i]=='ROL_LAVADO_DINERO_OPERADOR' and $ccuenta<>0))	{$habilitado=1;} 
	if (($_SESSION['rol'.$i]=='ROL_LAVADO_DINERO_ADMINISTRA'))	{$habilitado_valoracion=1; $habilita_apertura=1;} 
}
if ($habilitado==0){
...
<form id="novedad" name="novedad" action="#" onsubmit="ajax_post('contenido','cliente/adm_novedad_casino.php',this); return false;">
  <table width="40%"  align="center">
    <tr>
      <td colspan="8" align="center" valign="bottom" class="texto4" scope="col">Reporte Diario de Casinos</a></td>
    </tr>
    <tr>
      <td colspan="8" align="center" valign="bottom" class="td8max" scope="col"><?php if(substr(strtolower($soydeaca),0,6)=='casino'){ echo $soydeaca;} else {armar_combo($rs_casino,"casino",$casino);}?>
        </a></td>
    </tr>
    <tr valign="bottom" class="td8" >
      ...
      <?php //SOLICITUD DE REPORTE XX
	  if ($habilita_apertura==1) {?>
      <td width="26" align="center" class="td2" ><a href="#" onClick="window.open('list1/mov_detalle_observaciones.php?casino=<?php if(substr(strtolower($soydeaca),0,6)=='casino'){echo $casino;} else {?>'+novedad.casino.value+'<?php }?>&fechita='+novedad.fecha.value,'Ventana','width=400,height=400,top=0,Left=0,menubar=no,scrollbars=yes,resizable=yes')"><img src="image/ojos.jpg" title="Valoraciones Marcadas(X) - Diaria" width="20" height="20" border="0" /></a></td>
      <td width="26" align="center" class="td2" ><a href="#" onclick="g('cliente/valoracion_filtro.php','#contenido');return false;"><img src="image/Error.png" title="Valoraciones Marcadas(X) - Todas" width="24" height="23" border="0" /></a></td>
      <?php }?>
...
<table width="95%" border="0" align="center">
  ...
  <tr align="center" class="td2">
    ...
    <?php if($habilitado_valoracion==1)
	  	{?>
    <td width="5%"  class="td4" colspan="2">Valoracion</td>
    <?php }?>
  ...
  <tr class="<?php if ($_pagi_result->CurrentRow()%2) { echo "td";}  else {echo "td8";}?>">
    <td align="left"><?php 
		   if($row->NOMBRE<>'')
		   {
		  			 echo trim($row->APELLIDO).', '.$row->NOMBRE;
		   }
		   else
		   {
		   			 echo trim($row->APELLIDO);
		   }?></td>
    ...
    <td  align="left"><textarea name="observamov" class="small"  id="observamov"  rows="2" cols="40" readonly="readonly"/>
      <?php echo $row->NOVEDAD;?>
      </textarea></td>
    <?php if($habilitado_valoracion==1)
           	{ ?>
    <td width="6%"  align="center" ><?php  if ($row->COMPLETA==0){?>
      <a href="#" onclick="ajax_get('contenido','cliente/procesar_datos.php','id_id=<?php echo $row->IDID;?>&casino=<?php echo $casino;?>&fecha=<?php echo $fecha;?>&completa=1');return false;"><img src="image/SyncCenter.png" title="Datos Completos" width="24" height="23" border="0"/> </a>
      <?php } else {?>
      <a href="#" onclick="ajax_get('contenido','cliente/procesar_datos.php','id_id=<?php echo $row->IDID;?>&casino=<?php echo $casino;?>&fecha=<?php echo $fecha;?>&completa=0');return false;"><img src="image/Error.png" title="Datos Incompletos" width="24" height="23" border="0" /></a>
      <?php  }?>
    </td>
    <!--<td width="6%"  align="center" ><a href="#" onClick="ajax_get('detalle','alerta/detalle_alerta.php','fecha=<?php// echo $fecha;?>&documento=<?php// echo $row->DOCUMENTO;?>&id_tipo=<?php// echo $row->ALERT;?>&alerta=<?php// echo $row->NOMBRE_ALERTA;?>&descrip=<?php// echo urlencode($row->NOMBRE);?>&id_base=<?php // echo $row->IDID;?>&id_estado=<?php //echo $row->ID_ESTADO;?>&observaciones=<?php// echo $row->OBSERVACIONES;?>&aplicacion=1')"><img src="image/glass.png" TITLE="Detalle de Valoracion" width="20" height="20" border="0" /></a></td>-->
    <td width="6%" align="center"><a id="down_<?php echo $row->IDID; ?>" onClick="ver_historico_2('div2_<?php echo $row->IDID;?>','cliente/detalle_cliente.php','id_id=<?php echo $row->IDID;?>','<?php echo $row->IDID; ?>')"><img src="image/glass.png" TITLE="Detalle de Valoracion" width="20" height="20" border="0" /></a> </td>
    <?php }?>
    <?php if($row->CONFIRMA=='n' OR $row->CONFIRMA=='N')
		   {?>
    <td align="center"><a href="#" onclick="ajax_get('contenido','cliente/modificar_novedad.php','id_id=<?php echo $row->IDID;?>&casino=<?php echo $casino;?>&fecha=<?php echo $fecha;?>');return false;"><img src="image/modificar.png" title="Modifica este Movimiento?" width="20" height="20" border="0"/></a></td>
    <td align="center"><a href="#" onclick="ajax_get('elimina','cliente/controla_eliminacion_novedad_casino.php','id_id=<?php echo $row->IDID;?>&casino=<?php echo $casino;?>&fecha=<?php echo $fecha;?>');return false;"><img src="image/roseta_ok.png" title="Anula este Movimiento?" width="20" height="20" border="0"/></a></td>
    <?PHP }
		   else
		   {?>
    <td width="2%" align="center" colspan="2"><img src="image/candado.png" title="Imposible - Jornada Cerrada" width="20" height="20" border="0"/></td>
    <?php }
		   ?>
  </tr>
  <tr  id="tr2_<?php echo $row->IDID; ?>" style="background-color:#E2F4FF;">
    <td id="td2_<?php echo $row->IDID;?>" colspan="8"><div id="div2_<?php echo $row->IDID;?>" style="display:none"></div></td>
  </tr>
  <?php 
...
?>
  ...
  <tr align="center">
    <td align="center" scope="row" colspan="9"><div align="right"><img src="image/24px-Crystal_Clear_action_reload.png" title="Cerrar" width="16" height="16" border="0" align="absbottom" /> <a href="#" class="small" onclick="ajax_get('contenido','cliente/blanco.php','')">Cerrar</a></div></td>
  </tr>
</table>
