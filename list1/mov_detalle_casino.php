...
/*try {
	$rs_consulta = $db->Execute("SELECT  DECODE(cl.nombre,NULL,cl.apellido, cl.apellido
								  || ', '
								  || cl.nombre) AS datos,
								...
								FROM PLA_AUDITORIA.t_cliente cl,
								  PLA_AUDITORIA.t_novedad_casino g
...
try {
	$rs_consulta = $db->Execute("SELECT cl.apellido  || ' ' || cl.nombre 
										 as descripcion,
								  g.fichaje as fichaje,
								  g.acierto as acierto,
								  g.mon_ing_fic as fichaje_ingreso,
...
//$pdf->SetX(30);
$pdf->Cell(190,8,'Reporte Diario Fichaje - [Datos Resguardados] - Fecha '.$fechita,0,1,'C',1);
...
//$pdf->Ln(8);
	...
	//if($row->NOMBRE==NULL)
	//{
		$pdf->Cell(60,6,utf8_decode(utf8_decode(substr($row->DESCRIPCION,0,40))),1,0,'L');
	/*}
	else
	{
		$pdf->Cell(60,6,utf8_decode(trim($row->APELLIDO)).', '.utf8_decode(trim($row->NOMBRE)),1,0,'L');
	}*/
