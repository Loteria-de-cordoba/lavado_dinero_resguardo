...
var dhtmlXVaultObject = function(){
	...
	this.imPath = "imgs/";
	this.strings = {remove:"Quitar", upload:"Subir", done:"Listo", error:"ERROR", btnAdd:"Agregar", btnUpload:"Subir", btnClean:"Limpiar"};
...
dhtmlXVaultObject.prototype.create = function(htmlObject){
	...
	this.container.innerHTML = "<table class='dhxvlt_panelbg dhxvlt_panelborder' border='0'>" +
		...
		"<td style='width:180px;height:32px' align='left'>" +
 		"<div class='dhxvlt_lbtn'><span class='dhxvlt_lbtn1'></span><span class='dhxvlt_lbtn2'><nobr><img src='" + this.imPath + "upload.gif'/> "+this.strings.btnUpload+"</nobr></span><span class='dhxvlt_lbtn3'></span></div></td>" +
 		"<td style='width:136px;height:32px;' align='right'>" +
 		"<div class='dhxvlt_rbtn'><span class='dhxvlt_rbtn3'></span><span class='dhxvlt_rbtn2'><nobr><img src='" + this.imPath + "clean.gif'/> "+this.strings.btnClean+"</nobr></span><span class='dhxvlt_rbtn1'></span></div></td></tr></table>" +
 		"<div class='btnAddBtn dhxvlt_lbtn'><span class='dhxvlt_lbtn1'></span><span class='dhxvlt_lbtn2'><nobr><img src='" + this.imPath + "add.gif'/> "+this.strings.btnAdd+"</nobr></span><span class='dhxvlt_lbtn3'></span></div>" +
 		"<div class='btnAddDiv'>" +
 		"<input type='file' id='file1' name='file1' value='' class='dhxvlt_hidden " + (this.Opera ? "dhxvlt_fo" : "") + "'/></div>";
	this.parentObject.appendChild(this.container);
	var self = this;
	this.container.childNodes[0].rows[1].cells[1].childNodes[0].onclick = function() {self.uploadAllItems()};
	this.container.childNodes[0].rows[1].cells[2].childNodes[0].onclick = function() {self.removeAllItems()};
	this.fileContainer = this.container.childNodes[2];
	this.currentFile = this.fileContainer.childNodes[0];
	this.currentFile.onchange = function() {self.addFile()};
	if (this.IE){this.uploadForm = document.createElement("<form enctype='multipart/form-data' target='dhtmlxVaultUploadFrame' method='post'>")}else {this.uploadForm = document.createElement("form");this.uploadForm.method = "post";this.uploadForm.encoding = "multipart/form-data";this.uploadForm.target = "dhtmlxVaultUploadFrame"};
	...
	if (this.idRowSelected){
		...
		if (row){
			if (row.id != currentRow.id){
				...
				this.idRowSelected = currentId;row.className = "dhxvlt_row"
			}else {
				currentRow.className = "dhxvlt_row";this.idRowSelected = ""
			}
		}else {
			currentRow.className = "dhxvlt_rowsel";
...
dhtmlXVaultObject.prototype.addFile = function(){
	...
	rowListFiles.onclick = function(){self.selectItem(currentId)};
	...
	cellList.firstChild.onclick = cellList.childNodes[1].onclick = function(){self.removeItem(currentId)};
	cellList.vAlign = "bottom";
	cellList.align = "center";
	rowList.appendChild(cellList);
	...
	cellList.firstChild.onclick = function() {self.uploadFile(currentId);return false};
...
dhtmlXVaultObject.prototype.getImgFile = function(fileName){
	...
	if (valueImgPic.indexOf(ext)!= -1){return srcImgPic};
	if (valueImgVideo.indexOf(ext)!= -1){return srcImgVideo};
	if (valueImgSound.indexOf(ext)!= -1){return srcImgSound};
	if (valueImgArchives.indexOf(ext)!= -1){return srcImgArchives};
	return srcImgFile
};
dhtmlXVaultObject.prototype.createId = function(){return this.counter++};
	...
	if (this.tblListFiles != null){
		if (this.tblListFiles.rows.length > 0){
			for (var i = 0;i < this.tblListFiles.rows.length;i++){
				if (this.tblListFiles.rows[i].attributes["isUpload"].value == "false"){flag = i;break}
			};
			if (flag != -1){
					...
					if (this.isUploadFileAll)
						try {this.onUploadComplete(this.objToArray(this.fileList))}
						catch(e) {};
					this.fileList = {};
					this.isUploadFileAll = false
			}
...
dhtmlXVaultObject.prototype.endLoading = function (id, isError){
	...
	if (f){f.error = isError;f.uploaded = !isError};
	try {this.onFileUploaded(f)}catch(e) {};
	if (isError)try {this.onUploadComplete(this.objToArray(this.fileList))}catch(e) {};
...
dhtmlXVaultObject.prototype.startRequest = function(id){
	...
	if (xmlHttp.status == 200){
		if (!xmlHttp.responseText){throw "error"};
	...
	try {
		...
		if (xmlHttp.status == 200){
			...
			if (res){
				...
				if (res != -1){
					var self = this;
					try {window.setTimeout(function () {self.sendIdSession(id)}, 500)}catch(e){}
				}else if (res == -1){
					this.uploadedCount++;this.endLoading(id, false);
					this.showMessageInfo(id, this.strings.done);
					if (this.isUploadFileAll){this.uploadAllItems()}
				}
			}
		}else{throw "error"}}catch(e){
...
dhtmlXVaultObject.prototype.getFilesCount = function(){return this.tblListFiles && this.tblListFiles.rows ? this.tblListFiles.rows.length : 0};
dhtmlXVaultObject.prototype.getTotalFilesCount = function(){
	var count = this.uploadedCount;
	if (this.tblListFiles != null){
		if (this.tblListFiles.rows.length > 0){
			for (var i = 0;i < this.tblListFiles.rows.length;i++){
				if (this.tblListFiles.rows[i].attributes["isUpload"].value == "false"){count++}
			}
		}
	};
	return count
...
dhtmlXVaultObject.prototype.getCurrentRowListFiles = function(id){
	for (var i = 0;i < this.tblListFiles.rows.length;i++){
		if (this.tblListFiles.rows[i].id == "rowListFiles" + id){return this.tblListFiles.rows[i]}
	}
};
...
dhtmlXVaultObject.prototype.getFormField = function(type, name) {
	...
	for (var i = 0;i < fields.length;i++){
		var f = fields[i];
		if (f.type.toLowerCase()== type && f.name == name){return f}
	};
	return null
};
dhtmlXVaultObject.prototype.getCurrentInputFile = function(id){return this.getFormField("file", "file" + id)};
dhtmlXVaultObject.prototype.uploadFile = function (id){
	if (!this.isUploadFile){
		...
		try {this.startRequest(id);this.sendIdSession(id)}
		catch(e){
			this.endLoading(id, true);
 			this.isUploadFileAll = false;
 			this.isUploadFile = false;
...
dhtmlXVaultObject.prototype.setFilesLimit = function(limit) {var n = parseInt(limit); if (!isNaN(n)&& n >= 0) this.filesLimit = n};
dhtmlXVaultObject.prototype.setFormField = function(name, value) {if (!this.uploadForm){alert("Please call setFormField() method after create()!");return};var field = this.getFormField("hidden", name);if (value === null){if (field)this.uploadForm.removeChild(field)}else {if (!field){field = document.createElement("input");field.type = "hidden";field.name = name;this.uploadForm.appendChild(field)};field.value = value}};
dhtmlXVaultObject.prototype.onAddFile = function(fileName) {return true};
