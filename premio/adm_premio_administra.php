...
{
//EJERZO AUDITORIA
	 $describa='Siendo las '.$serhora.' horas,  El Agente '.$auditado.' Ingresa a Pantalla de  Ganadores--->[Resguardo]';
...
/*if (isset($_GET['fecha']) or isset($_GET['fdesde'])) {
	$fecha = $_GET['fecha'];
	} else {
		if (isset($_POST['fecha']) or isset($_POST['fdesde'])) {
			$fecha = $_POST['fecha'];
			} else {
				$fecha = '01/'.str_pad($array_fecha["mon"],2,'0',STR_PAD_LEFT).'/'.$array_fecha["year"];
...
//valor a enviar listado de registro premios pagados
$mesano = substr($fecha,3,2).'/'.substr($fecha,6,4); 
...
//$fhasta='31/08/2011';
 ...
 $_pagi_sql ="select rownum,a.id_ganador, to_char(a.fecha_alta,'DD/MM/YYYY') as fecha, upper(a.nombre) nombre , observacion, a.politico, 
			...
			upper(substr(d.descripcion,1,13)) usuario, upper(substr(e.descripcion,1,13)) usuario_conforma,upper(substr(v.descripcion,1,13)) usuario_modifica,a.ddjj,
			(select count(*) 
				from PLA_AUDITORIA.ARCHIVOS_PLA x 
				where x.esquema='lavado_dinero' and x.tabla='t_ganador' and x.id_tabla = a.id_ganador) as imagenes,
			a.xml,
 			 tp.descripcion as ppago
      		from PLA_AUDITORIA.t_ganador a, juegos.sucursal b, superusuario.usuarios d, juegos.juegos c,
			superusuario.usuarios e,superusuario.usuarios v,
  			PLA_AUDITORIA.t_tipo_pago tp
			where a.suc_ban = b.suc_ban(+) 
			and a.juego = c.id_juegos
			and a.usuario = d.id_usuario
			and a.usuario_conforma= e.ID_usuario(+)
			and a.usuario_modifica= v.ID_usuario(+)
			and a.id_tipo_pago=tp.id_tipo_pago
...
$_pagi_cuantos = 5000; //OPCIONAL. Entero. Cantidad de registros que contendrá como máximo cada página. Por defecto está en 20.
...
<table width="100%" border="0" align="center">
	<tr>
    	<td colspan="14" align="center" valign="bottom" class="texto4" scope="col">
            ...
            Descargar Planillas <img src="image/jamembo-jumpto.png" title="Descargar Planillas" width="28" height="28" border="0" /></a>
		</td>
	</tr>
	<tr>
        <td width="4%" align="right"> <a href="#" onclick="window.open('list/personas_politicas.php?conformado=<?php echo $conformado; ?>&fecha=<?php echo $fecha;?>&fhasta=<?php echo $fhasta;?>&suc_ban=<?php echo $suc_ban;?>&mayores=<?php echo $mayores;?>','Ventana','width=400,height=400,top=0,Left=0,menubar=no,scrollbars=yes,resizable=yes')"><img src="image/24px-Crystal_Clear_app_printer.png" title="Imprimir" width="24" height="23" border="0" /></a></td>
      <td colspan="9" align="center" valign="bottom" scope="col" class="texto4">Datos Personales de Premios[Datos Resguardados]</td>
        <td width="8%" align="center"> <a href="#" onclick="window.open('list/premios_pagados.php?conformado=<?php echo $conformado; ?>','Ventana','width=400,height=400,top=0,Left=0,menubar=no,scrollbars=yes,resizable=yes')"><img src="image/24px-Crystal_Clear_app_printer.png" title="Imprimir" width="24" height="23" border="0" /></a>&nbsp;&nbsp;&nbsp;<a href="#" onclick="window.open('list/premios_pagados_registro.php?conformado=<?php echo $conformado; ?>&mesano=<?php echo $mesano;?>','Ventana','width=400,height=400,top=0,Left=0,menubar=no,scrollbars=yes,resizable=yes')"><img src="image/Adobe Reader 7.png" title="Registro Premios Mayores $50.000" width="24" height="23" border="0" /></a>
        <?php if($conformado==1)
		{?>
       <!-- &nbsp;&nbsp;&nbsp; <a href="#" onclick="window.open('list/premios_pagados_fuera_de_termino.php?conformado=<?php echo $conformado; ?>&fecha_hasta=<?php echo $fhasta; ?>&fecha_desde=<?php echo $fecha; ?>','Ventana','width=400,height=400,top=0,Left=0,menubar=no,scrollbars=yes,resizable=yes')"><img src="image/relok.jpg" title="Conformado fuera de termino" width="24" height="23" border="0" /></a>
		--><?php }?>        </td>
    </tr>
    <tr>          
        <td colspan="11" align="center" valign="bottom" class="smallRojo" scope="col"><?php echo $_pagi_navegacion ."   ".$_pagi_info ?></td>
    <!--    <td colspan="5" align="center" valign="bottom" class="smallRojo" scope="col">
        ...
        <td align="center" valign="bottom" class="smallRojo" scope="col">
            <input type="submit" class="small" style="width:75px" value="Generar xml" />
        </td>-->
    </tr>   
	<tr align="center" class="td2">
    	
		<td width="4%" class="td4" scope="col">Fecha</td>
      <td width="10%" class="td4" scope="col">Sucursal</td>
      <td width="15%" class="td4" scope="col">Ganador</td>
      <td width="4%" class="td4" scope="col">Juego</td>
      <td width="5%" class="td4" scope="col">Importe</td>
      <td width="10%" class="td4" scope="col">Im&aacute;genes</td>
    ...
    <!--<td width="18%" class="td4" scope="col">Usuario Conforma</td>-->
        <td width="5%" class="td4" scope="col">Datos Ganador</td>
      <!--<td width="7%" class="td4" scope="col">Observación</td>
      <td width="4%" class="td4" scope="col">Modif.</td>
		<td width="12%" class="td4" scope="col">PEP'S</td>
        <td width="4%" class="td4" scope="col">Elim.</td>-->
      <td scope="col">Estado<br />xml</td>
        <!--<td width="5%" scope="col">Generar<br />
      xml</td>-->
    </tr>
	<?php while ($row = $_pagi_result->FetchNextObject($toupper=true)){?>
		<div class="cloud"><div class="contenido" style=" font-weight:bold; font-size:24px; color:#FF0000; text-align:center"><p>Color Rojiso - Ganador PEPS</p></div></div>
        <div class="nube"><div class="contenido" style=" font-weight:bold; font-size:24px; color:#FF0000; text-align:center"><p>Premio>=1.000.000</p></div></div>	  
        <tr  class="<?php  if($row->POLITICO=="SI"){echo "tdrojo";} else if($row->VALOR_PREMIO>=1000000) {echo "tdorange";} else if($_pagi_result->CurrentRow()%2) { echo "td";}  else {echo "td8";}?>">
			<td align="center" onmouseover="<?php if($row->POLITICO=="SI"){?>$('.cloud').toggle(); center('.cloud');<?php } else if($row->VALOR_PREMIO>=1000000){?>$('.nube').toggle(); center('.nube');<?php } else{$zzz=0;}?>" onmouseout="<?php if($row->POLITICO=="SI"){?>$('.cloud').toggle();<?php }else if($row->VALOR_PREMIO>=1000000){?>$('.nube').toggle();<?php }else{$zzz=0;}?>"><?php echo $row->FECHA;?></td>
         	...
         	<td align="left" ><?php echo $row->JUEGOS; $totales=$totales+$row->VALOR_PREMIO?></td>
           	...
           	<td align="center" >
                ...
                <img src="image/download.png" title="ver archivos"  width="20" height="20" border="0"/> 
                </a>
			</td>
          ...
          <td width="12%" align="left" style="font-size:10px" ><?php echo $row->USUARIO_MODIFICA;?></td>
       	  ...
       	  <td align="center" ><a href="#" onclick="window.open('list/datos_ganadores.php?id_ganador=<?php echo $row->ID_GANADOR ?>&delegacion=<?php echo utf8_decode($row->SUCURSAL)?>','Ventana','width=400,height=400,top=0,Left=0,menubar=no,scrollbars=yes,resizable=yes')"><img src="image/Adobe Reader 7.png" title="Imprimir" width="24" height="23" border="0" /></a></td>
		  <!--<td align="center" >
		 	<?php// if ($row->OBSERVACION==0){?>
         		<a href="#" onclick="ajax_get('contenido','premio/procesar_datos.php','id_ganador=<?php// echo $row->ID_GANADOR ?>&fdesde=<?php// echo $fecha; ?>&fhasta=<?php/ echo $fhasta; ?>&observacion=1&conformado=<?php// echo $conformado; ?>&suc_ban=<?php// echo $suc_ban ; ?>')"><img src="image/SyncCenter.png" title="Datos Completos" width="24" height="23" border="0" />
                </a>
          		<?php// } else {?>
		   ...
		   <?php  //}?>
           <a href="#" onclick="ajax_get('contenido','premio/nueva_nota_observacion.php','id_ganador=<?php echo $row->ID_GANADOR ?>&fdesde=<?php echo $fecha; ?>&fhasta=<?php echo $fhasta; ?>&conformado=<?php echo $conformado ; ?>&suc_ban=<?php echo $suc_ban; ?>')"><img src="image/app_48.png" title="Añadir Nota" width="24" height="23" border="0" /></a>   
           </td>
           <td align="center" ><a href="#" onclick="ajax_get('contenido','premio/modificar_premio2.php','id_ganador=<?php echo $row->ID_GANADOR ?>&fdesde=<?php echo $fecha; ?>&fhasta=<?php echo $fhasta; ?>&conformado=<?php echo $conformado ; ?>&casa=<?php echo $suc_ban; ?>&mayores=<?php echo $mayores; ?>'); return false;" ><img src="image/modificar.png" title="modificar datos"  width="20" height="20" border="0"/></a></td>
        ...
        <?php// } else {?> NO <?php//  }?>  </td>
            <td align="center" ><a href="#" onclick="confirmar_eliminar_ganador('contenido','premio/procesar_eliminar_ganador.php','id_ganador=<?php// echo $row->ID_GANADOR ?>&fdesde=<?php// echo $fecha; ?>&fhasta=<?php// echo $fhasta; ?>&conformado=<?php/ echo $conformado ; ?>&casa=<?php/ echo $suc_ban; ?>','')" ><img src="image/Trash-Empty.png" title="Eliminar Ganador"  width="20" height="20" border="0"/></a></td>-->
            <td align="center">
            	<?php if ($row->XML == 1) {
                	echo "<img src='image/si.png' title='Generado'  width='20' height='20' border='0'/>";
                    } else {
                    	echo "<img src='image/no.png' title='Sin Generar'  width='20' height='20' border='0'/>";
                    	} ?>
            </td>
        	<!--<td align="center">
