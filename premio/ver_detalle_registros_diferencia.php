...
<form id="form1" name="form1" method="post" onsubmit="ajax_post('conformar','premio/procesar_conformar_dif.php',this); return false;">
  ...
  <input name="sucusucu" type="hidden" value="<?php echo $suc_ban;?>" />
  <span class="td2">
  <?php if (!is_null($rowgana->FECHA)) {echo $rowgana->FECHA; $fecha=$rowgana->FECHA; }else{ echo ' ';}?>
  </span>  <table width="65%" border="1" cellspacing="0" cellpadding="0" align="center" >
      <tr>
    	<td align="right" class="fondo1_small" colspan="2"> <img src="image/24px-Crystal_Clear_app_kjobviewer.png" width="24" height="24" border="0" onclick="window.open('list/lista_registros_diferencia2.php?fdesde=<?php echo $fdesde; ?>&fhasta=<?php echo $fhasta; ?>&sucusucu=<?php echo $suc_ban; ?>', '_blank', 'height=480, width=640, left=0, top=0, toolbar=no, menubar=no, titlebar=no, resizable=yes, scrollbars=yes')"/> Imprimir registros </td>
   </tr>      
      <tr>
      ...
      <tr>
		...
		<tr>
			<td width="15%" align="center" class="td9Grande">Item</td>
           <td width="15%" align="center" class="td9Grande">Fecha</td>
            <td width="25%" align="center" class="td9Grande">Monto</td>
            <td width="25%" align="center" class="td9Grande">Ver Detalle</td>
...
while ($i<$_SESSION['cantidadroles'])  {
	...
	$i=$i+1;
if ($_SESSION['rol1']=='ROL_LAVADO_DINERO_ADM_SIN_CC'){
$sucursal=$_SESSION['suc_ban'];
$sucursales="and c.suc_ban=$sucursal";
} else 
{
		if ($_SESSION['rol1']=='LAVADO_DINERO_CONFORMA_TODO')
		{
			$sucursales="and c.suc_ban in $suc_ban";
		}
		else
		{
			$sucursal=$_SESSION['suc_ban'];
 ...
 if (($_SESSION['suc_ban']==1) and ($_SESSION['rol1']<>'ROL_LAVADO_DINERO_ADM_SIN_CC') and ($_SESSION['rol1']<>'LAVADO_DINERO_CONFORMA_TODO') and ($_SESSION['rol1']<>'ROL_LAVADO_DINERO_ADM_CASINO') and ($_SESSION['rol'.$i]!='ROL_LAVADO_DINERO_ADM_CONFORMA')) { 
                    ...
                    try {$rsconta = $db->Execute("select a.total, to_char(a.fecha_valor,'DD/MM/YYYY') as fecha_valor, a.nro_asiento, rownum 
                                              ...
                                              where a.cod_area=c.cod_area
												  ...
												  and a.fecha_valor between to_date('$fdesde','DD/MM/YYYY HH24:MI') and to_date('$fhasta','DD/MM/YYYY HH24:MI')
												  and ( upper(a.concepto) like '%UIF%' or upper(a.concepto) like '%U.I.F.%' )
												  order by fecha_valor, total desc");
            ...
            $fecha='';	
			 ...
			 ?>      
                  <tr>
				  	<td align="center" height="19" class="td2"><?php echo $i ?></td>
        ...
        <td valign="top" class="fondo1"><table align="left" width="101%" border="0" cellspacing="0" cellpadding="0">
	<tr>
                <td width="14%" align="center" class="td9Grande">Item</td>
				<td width="14%" align="center" class="td9Grande">Fecha</td>
              <td width="15%" align="center" class="td9Grande">Monto</td>
          <td width="22%" align="left" class="td9Grande"><div align="center">Nota</div></td>
          <td align="left" class="td9Grande" colspan="3"><div align="center">Conforma</div></td>
        </tr>
         <?php 
if ($_SESSION['rol1']=='ROL_LAVADO_DINERO_ADM_SIN_CC'){
$sucursal=$_SESSION['suc_ban'];
$sucursales="and suc_ban ='$sucursal')";
}  else 
{
		if ($_SESSION['rol1']=='LAVADO_DINERO_CONFORMA_TODO')
		{
			$sucursales="and suc_ban in $suc_ban";
		}
		else
		{
			$sucursal=$_SESSION['suc_ban'];
            ...
            try {$rsgana = $db->Execute("select valor_premio, to_char(fecha_alta,'dd/mm/yyyy') fecha, id_ganador, conformado 
                                              ...
                                              where fecha_alta between to_date('$fdesde 00:00','DD/MM/YYYY HH24:MI') and to_date('$fhasta 23:59','DD/MM/YYYY HH24:MI')
                                              $sucursales
            ...
            $fecha=''; 
			$i=0; 
		if($rsgana->recordCount()==0){
			die();
			} else {	
        while ($rowgana=$rsgana->FetchNextObject($toupper=true)) {
		$i=$i+1;
		//echo 'FECHAAAAAA '.$rowgana->FECHA;
		//die();
		?>
              <tr>
			  	<td align="center" height="21" class="td2"><?php echo $i?></td>
      ...
      if($rowgana->CONFORMADO=='0') {
		...
		$rowcantidad=$rsdatos->FetchNextObject($toupper=true);
			if ($rowcantidad->CANTIDAD==0){
				$chequeado="";
				$habilitado="";  ?>
                <td width="18%" align="center" class="td2">
	...
	<?php	}
					...
					{	// cuando faltan cargar datos en la alta del ganador!
                        ...
                        <td width="17%" align="center" class="td2">
                        <a href="#" onclick="ajax_get('contenido','premio/modificar_premio.php','id_ganador=<?php echo $rowgana->ID_GANADOR; ?>&bandera=2');"><img src="image/pinche.gif" title="Completar datos del ganador!!"  width="28" height="25" border="0" /></a>
                <input name="conformadif<?php echo $i; ?>"  type="checkbox" id="conformadif<?php echo $i; ?>" value="<?php echo $rowgana->ID_GANADOR; ?>" <?php echo $habilitado; ?> <?php echo $chequeado;?> title="seleccionar" align="rigth" />				</td>
					<?php }
		}
      ...
      <tr>
        	...
        	<td align="right" class="fondo1_small">  </td>
        	<td align="right" class="fondo1" colspan="4"><div align="center">
...
<input name="fhasta" type="hidden" value="<?php echo $fhasta; ?>" />
    ...
    <table width="81%" border="1" align="center" cellspacing="0" cellpadding="0">
      <tr>
          ...
          <!--<td width="31%" align="left" class="td9Grande">Ver Detalle</td>-->
            ...
            <?php   	
                    try {$rsconta = $db->Execute("SELECT importe_plata AS total, importe_impuesto as IMPUESTO , id_cp, importe_ficha ,
												  ...
												  AND fecha BETWEEN to_date('$fdesde 00:00','DD/MM/YYYY HH24:MI') AND to_date('$fhasta 23:59','DD/MM/YYYY HH24:MI')
												  AND importe_ficha>=25000
            ...
            $fecha='';	
             while ($rowconta=$rsconta->FetchNextObject($toupper=true)) {?>      
                  <tr>
                    <td align="right" height="19" class="td2"><?php if (!is_null($rowconta->FECHA_VALOR)) {echo $rowconta->FECHA_VALOR; $fecha=$rowconta->FECHA_VALOR;}else{echo ' '; } ?></td>
...
if (isset($_GET['suc_ban']) && $_GET['suc_ban']!=0) {
	...
	$condicion_sucursal = "and (suc_ban in ($suc_ban) or suc_ban=81)";	
		...
		$i=$i+1;?>
              <tr>
                <td align="center" height="21" class="td2"><?php if (!is_null($rowgana->FECHA)) {echo $rowgana->FECHA; $fecha=$rowgana->FECHA; }else{ echo ' ';}?></td>
                <td align="right" class="td2"><?php echo '$ '.number_format($rowgana->VALOR_PREMIO,2,',','.'); ?></td>
                <td align="center" class="td2">
                <a href="#" onmouseover="ajax_showTooltip('premio/nueva_nota.php?jsfecha='+new Date()+'&amp;fdesde=<?php echo $fdesde; ?>&amp;fhasta=<?php echo $fhasta; ?>&amp;ganador=<?php echo $rowgana->ID_GANADOR; ?>',this);return false" onmouseout="ajax_hideTooltip()"><img src="image/xmag.gif" alt="ver" width="23" height="21" border="0" /></a>
                <a href="#" onclick="ajax_get('contenido','premio/nueva_nota.php','ganador=<?php echo $rowgana->ID_GANADOR; ?>&fdesde=<?php echo $fdesde; ?>&fhasta=<?php echo $fhasta; ?>');"><img src="image/app_48.png" alt="agregar nota"  width="23" height="21" border="0"/></a></td>
		...
		$rowcantidad=$rsdatos->FetchNextObject($toupper=true);
			...
			<?php	}
					...
					{	// cuando faltan cargar datos en la alta del ganador!
						...
						$habilitado='disabled="disabled"';	?>
                        <td align="center" class="td2">
                        <a href="#" onclick="ajax_get('contenido','premio/modificar_premio2.php','id_ganador=<?php echo $rowgana->ID_GANADOR; ?>&bandera=2');"><img src="image/pinche.gif" title="Completar datos del ganador!!"  width="28" height="25" border="0" /></a>
                        <input name="conformadif<?php echo $i; ?>"  type="checkbox" id="conformadif<?php echo $i; ?>" value="<?php echo $rowgana->ID_GANADOR; ?>" <?php echo $habilitado; ?> <?php echo $chequeado;?> title="seleccionar" align="rigth" />						</td>
					<?php }
		}
             else { $chequeado='checked="checked"';
