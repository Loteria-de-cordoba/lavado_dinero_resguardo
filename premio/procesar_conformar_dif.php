...
$fhasta=substr($_POST['fhasta'],0,10);
	if(isset($_REQUEST['sucusucu']) and $_REQUEST['sucusucu']<>0)//UNICAMENTE SI VIENE EL ROL DE NEHME
	{
		$suc_ban=$_REQUEST['sucusucu'];
		$condicion_suc=" and suc_ban=$suc_ban";	
	}
...
//obtengo el nombre del apostador
 ...
 $rs_uu11 = $db ->Execute("SELECT ga.apellido || ' ' || ga.nombre as apenom
  ...
  //$describa='CARGA GANADOR: Siendo las '.$serhora.' horas,  El Agente '.$auditado.' CARGA movimiento del Sr. '.$apostador.' en '.$cas_audita.' monto: '.$_POST['valor_premio'].' - fecha '.date('d/m/Y');
...
//die('en proceso');	
header ("location:mostrar_pre_conformacion.php?fdesde=$fdesde&fhasta=$fhasta&suc_ban=$suc_ban");?>
