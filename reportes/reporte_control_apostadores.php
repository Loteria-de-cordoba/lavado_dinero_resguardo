...
//id_sistema=75 - - CONTROL DE APOSTADORES
...
try {
	...
	FROM 
		...
		superusuario.sistemas si,
		superusuario.usuario_x_sistema uxs,
  		rrhh.t_empleado emp
	WHERE US.ID_USUARIO=D.GRANTEE
		...
		AND us.id_usuario=uxs.id_usuario
		AND uxs.id_sistema=si.id_sistema
		AND si.id_sistema=75
		AND EMP.ACTIVO='S'
	ORDER BY AR.DESCRIPCION, US.DESCRIPCION
	");
} catch  (exception $e) {
	die($db->ErrorMsg());
}
...
$titulo = 'Areas y Roles de Empleados Usuarios del Sistema CONTROL DE APOSTADORES';
...
while ($row = $rs_datos->FetchNextObject($toupper=true)) {
	...
	if ($salto_pagina > 180) {
		...
		if ($pri=='NO') {
			...
			$pdf->SetFont('Arial','B',11);
			$pdf->Cell(10,6,'',0,0,'C');
			$pdf->Cell(85,6,utf8_decode(substr($row->AREA,0,41)),0,0,'L',1);
