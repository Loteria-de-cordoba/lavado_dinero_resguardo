...
try {
 $rs_estado_alerta = $db ->Execute("SELECT id_estado_alerta as codigo, 
 							descripcion as descripcion 
 							FROM PLA_AUDITORIA.estado_alerta
...
if(isset($_REQUEST['id_estado'])&& $_REQUEST['id_estado']<>0)
{
	...
	$condicion_estado="and b.id_estado_alerta=$id_estado";
}
else
{
	$id_estado=0;
...
//{
//EJERZO AUDITORIA
	 $describa='Siendo las '.$serhora.' horas,  El Agente '.$auditado.' consulta Alertas--->[Resguardo]';
...
$_pagi_div = "contenido";
$_pagi_enlace = "alerta/adm_alerta.php";
...
<form id="novedad" name="novedad" action="#" onsubmit="ajax_post('contenido','alerta/adm_alerta.php',this); return false;">
  <table width="68%"  align="center">
    <tr>
      <td colspan="13" height="25" align="center" valign="bottom" class="texto4" scope="col">MATRIZ de RIESGO - ADMINISTRACION de ALERTAS [Datos  Resguardados]</td>
    </tr>
    <tr valign="bottom" class="td8" >
      <td align="right" colspan="8" valign="middle" class="td2"  scope="col" style="text-align:right; background-color:#FFFFFF">&nbsp;</td>
      <td align="right"  valign="middle" class="td2"  scope="col" style="text-align:right">Fecha:&nbsp;&nbsp;
        <?php  echo $fecha;?></td>
    </tr>
    <tr valign="bottom" class="td8" >
      <td     width="87" align="center" class="td2">Filtros:</td>
      <td width="88" align="right" valign="middle" class="td2"  scope="col">Tipo de Alerta:</td>
      <td width="141"  valign="middle" class="td2" scope="col" align="left"><?php armar_combo_todos($rs_tipo_alerta,'tipo',$tipo);?></td>
      <td width="122" align="right" valign="middle" class="td2"  scope="col">Estado de Alerta:</td>
      <td width="76"  valign="middle" class="td2" scope="col" align="left"><?php armar_combo_todos($rs_estado_alerta,'id_estado',$id_estado);?></td>
     <td width="123"  align="center" class="td2" scope="col"><a href="#" onClick="ajax_get('contenido','alerta/adm_tipo_alerta.php','fecha=<?php echo $fecha;?>')"><img src="image/alerta.png" TITLE="Administracion de Tipos de Alertas" width="25" height="25" border="0"/></a>
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
     <a href="#" onclick="ajax_get('contenido','alerta/adm_alerta_anterior.php','fecha=<?php echo $fecha;?>'); return false;"><img src="image/Error.png" border="0" title="Alertas de Periodos Anteriores" width="25" height="25"/></a></td>
     
     <td width="0"></td>
      <!--en reemplazo va el siguiente cuando se habilite el de arriba lo saco
      
      <td width="50"  align="center" class="td2" scope="col">&nbsp;</td>-->
      <td width="128" align="center" class="td2" >
      <a href="#" onClick="window.open('list1/datos_alerta.php?condicion=<?php echo $condicion_tipo;?>&fecha=<?php echo $fecha;?>&condicion_estado=<?php echo $condicion_estado;?>','Ventana','width=400,height=400,top=0,Left=0,menubar=no,scrollbars=yes,resizable=yes')"><img src="image/printer.png" TITLE="Ver Reporte" width="25" height="25" border="0" /></a>
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      <a href="#" onClick="ajax_get('historico','alerta/historico.php','fecha=<?php echo $fecha;?>')"><img src="image/historico.png" TITLE="Resumen de Alertas Diario" width="25" height="25" border="0" /></a><!--<input name="estadistico" id="estadistico" type="button" style="width:2px;height:5px;" onclick="ajax_get('contenido','alerta/estadistico.php','fecha=<?php// echo $fecha;?>')"/>--></td>
      
      <!--<input type="hidden" name="fhasta_consulta" id="fhasta_consulta" value="<?php// echo $fhasta_consulta; ?>" />-->
      <td width="79" class="td2" scope="col" align="right"><input type="submit" style="font-weight:bold" value="Consultar" /></td>
    </tr>
...
{?>
		...
		<?php die("<br><div align=\"center\"><span class=\"textoRojo\">NO EXISTEN ALERTAS</span> <a href=\"#\" class=\"Estilo3\" onclick=\"ajax_get('contenido','blanco.php','')\">Cerrar</a></div>");?>
...
{?>
<tr>
    <td colspan="9" align="center" valign="bottom" style="background-color:#FF33FF; font-size:15px; border-color:#000000; color:#000000" scope="col"><b><?php echo $_REQUEST['mensaje'];?></b></td>
  </tr>
  ...
  <tr align="center" class="td2">
    ...
    <td width="9%"   class="td4" scope="col">Fecha de Aparici&oacute;n</td>
    <td width="14%"   class="td4" scope="col" style="font-style:italic"><b>Items<br /> 
    que Ocasiona el Alerta</b></td>
  ...
  ?>
     ...
     <td   align="center" style="font-size:11px"><?php echo $row->FEFE;?></td>
   ...
   <!-- &descrip=<?php// echo $row->NOMBRE;?>&alerta=<?php// echo $row->NOMBRE_ALERTA;?>-->
    ...
    <td width="6%"  align="center" ><a href="#" onClick="ajax_get('detalle','alerta/detalle_alerta.php','fecha=<?php echo $fecha;?>&documento=<?php echo $row->DOCUMENTO;?>&id_tipo=<?php echo $row->ALERT;?>&alerta=<?php echo $row->NOMBRE_ALERTA;?>&descrip=<?php echo urlencode($row->NOMBRE);?>&id_base=<?php echo $row->IDID;?>&id_estado=<?php echo $row->ID_ESTADO;?>&observaciones=<?php echo $row->OBSERVACIONES;?>&aplicacion=1')"><img src="image/glass.png" TITLE="Detalles y Actualizacion  del Alerta" width="20" height="20" border="0" /></a></td>
...
<div id="detalle">&nbsp;</div>
  ...
  <tr>
    ...
    <td colspan="3" align="center" valign="bottom" class="td8max" scope="col" style="background-color:#6666FF; font-size:16px;"><b><?php echo $toto;?></b></td>
  </tr>
  <tr>
    <td colspan="9" align="center" valign="bottom" class="smallRojo" scope="col"><?php echo $_pagi_navegacion ."   ".$_pagi_info ?></td>
  </tr>
  <tr>
    <td colspan="10" align="center" valign="bottom" class="smallRojo" scope="col"><div id="elimina">&nbsp;</div></td>
  </tr>
  <tr align="center">
    <td align="center" scope="row" colspan="9"><div align="right"><img src="image/24px-Crystal_Clear_action_reload.png" alt="Cerrar" width="16" height="16" border="0" align="absbottom" /> <a href="#" class="small" onclick="ajax_get('contenido','alerta/blanco.php','')">Cerrar</a></div></td>
  </tr>
