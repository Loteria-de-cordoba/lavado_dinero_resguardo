...
//actualizo base de alertas
ComenzarTransaccion($db);
			try {
				$db->Execute("update lavado_dinero.base_alerta
								set observaciones=?,
								id_estado_alerta=?
							WHERE 	id_base=?", array($observaciones,$id_estado,$id_base));				
...
//inserto en historico
						...
						try {
						$db->Execute("insert into lavado_dinero.estado_x_alerta(ID_BASE,
																   ID_ESTADO_ALERTA,
																  FECHA_MOD
																	)																   
							  values (?,?,sysdate)",
...
try {
 ...
 $rs_uu = $db ->Execute("SELECT us.descripcion as uu FROM 
	 ...
	 $describa='Siendo las '.$serhora.' horas,  El Agente '.$auditado.' MODIFICA BASE DE ALERTAS en  fecha '.$serfecha;
...
if($aplica==1)
	{
		header ("location:adm_alerta.php?mensaje=$mensaje");
	}
else
	{
		header ("location:adm_alerta_anterior.php?mensaje=$mensaje");
	}
?>
