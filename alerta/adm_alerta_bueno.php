...
try {
 $rs_tipo_alerta = $db ->Execute("SELECT id_tipo_alerta as codigo, 
 							...
 							FROM lavado_dinero.tipo_alerta
							ORDER BY 2		
					");
			}							
			catch (exception $e)
...
{
//EJERZO AUDITORIA
	 $describa='Siendo las '.$serhora.' horas,  El Agente '.$auditado.' consulta Alertas';
...
ComenzarTransaccion($db);			
			...
			try {
				$db->Execute("call lavado_dinero.ACTUALIZAR_ALERTAS_1()");
				}
				catch  (exception $e) 
				{ 
				die($db->ErrorMsg());
...
$_pagi_cuantos = 9; //OPCIONAL. Entero. Cantidad de registros que contendrá como máximo cada página. Por defecto está en 20.
...
<form id="novedad" name="novedad" action="#" onsubmit="ajax_post('contenido','alerta/adm_alerta.php',this); return false;">
  <table width="63%"  align="center">
    <tr>
      <td colspan="13" height="50" align="center" valign="bottom" class="texto4" scope="col">Administracion de Alertas</td>
    </tr>
    <tr valign="bottom" class="td8" >
      <td align="right" colspan="6" valign="middle" class="td2"  scope="col" style="text-align:right; background-color:#FFFFFF">&nbsp;</td>
      <td align="right"  valign="middle" class="td2"  scope="col" style="text-align:right">Fecha:&nbsp;&nbsp;
        <?php  echo $fecha;?></td>
    </tr>
    <tr valign="bottom" class="td8" >
      <td     width="92" align="center" class="td2">Filtros:</td>
      <td width="94" align="right" valign="middle" class="td2"  scope="col">Tipo de Alerta:</td>
      <td width="180"  valign="middle" class="td2" scope="col" align="left"><?php armar_combo_todos($rs_tipo_alerta,'tipo',$tipo);?></td>
       <td width="122" align="right" valign="middle" class="td2"  scope="col">Estado de Alerta:</td>
      <td width="113"  valign="middle" class="td2" scope="col" align="left"><?php armar_combo_todos($rs_estado_alerta,'id_estado',$id_estado);?></td>
      <td width="105" align="center" class="td2" >
      ...
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      <a href="#" onClick="ajax_get('historico','alerta/historico.php','fecha=<?php echo $fecha;?>')"><img src="image/historico.png" TITLE="Resumen de Alertas Diario" width="28" height="28" border="0" /></a>      </td>
      
      <!--<input type="hidden" name="fhasta_consulta" id="fhasta_consulta" value="<?php// echo $fhasta_consulta; ?>" />-->
      <td width="81" class="td2" scope="col" align="right"><input type="submit" style="font-weight:bold" value="Consultar" /></td>
    </tr>
  </table>
</form>
...
<tr>
  ...
  <tr align="center" class="td2">
    <td width="14%"   class="td4" scope="col">Tipo de Alerta</td>
    <td width="11%"   class="td4" scope="col">Estado</td>
    ...
    <td width="6%"   class="td4" scope="col">Actualizacion</td>
  ...
  ?>
  	...
  	<td   align="left" style=" font-weight:bold; background-color:<?php echo $color;?>"><?php echo $row->ESTADO;?></td>
   ...
   <!-- &descrip=<?php// echo $row->NOMBRE;?>&alerta=<?php// echo $row->NOMBRE_ALERTA;?>-->
    ...
    <td width="6%"  align="center" ><a href="#" onClick="ajax_get('detalle','alerta/detalle_alerta.php','fecha=<?php echo $fecha;?>&documento=<?php echo $row->DOCUMENTO;?>&id_tipo=<?php echo $row->ALERT;?>&alerta=<?php echo $row->NOMBRE_ALERTA;?>&descrip=<?php echo urlencode($row->NOMBRE);?>&id_base=<?php echo $row->IDID;?>&id_estado=<?php echo $row->ID_ESTADO;?>&observaciones=<?php echo $row->OBSERVACIONES;?>')"><img src="image/glass.png" TITLE="Detalles y Actualizacion  del Alerta" width="20" height="20" border="0" /></a></td>
