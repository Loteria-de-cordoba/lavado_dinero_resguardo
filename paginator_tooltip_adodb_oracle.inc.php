...
**	Paginator 
**	Paginación äd resultados ää!consultas a MySql con PHP
**
**	Versión 1.6.1
**
**	Nombre de archivo	:
 ...
 */
 if(!isset($_pagi_div)){
	// $_pagi_div no esta especificada la div de destino sera zona_contenido
	$_pagi_div = 'contenido';
 }
 if(empty($_pagi_sql)){
 ...
 */
 ...
 if($_pagi_conteo_alternativo == false){
	...
	try {
		$_pagi_result2 = $db->Execute($_pagi_sqlConta, $variables);
		}
		catch  (exception $e) 
		{ 
	...
	try {
		$_pagi_result3 = $db->Execute($_pagi_sql, $variables);
		}
		catch  (exception $e) 
		{ 
		die(MensajeBase($db->ErrorMsg()));
...
// $_pagi_query_string = "?";
 ...
 foreach($_pagi_propagar as $var){
 	if(isset($var)){
		// Si la variable es global al script
		eval("\$asignacion = \$".$var.";");//Codigo modificado
		$_pagi_query_string.= $var."=".$asignacion."&";//Codigo modificado
	}elseif(isset($var)){
		// Si no es global (o register globals está en OFF)
		$_pagi_query_string.= $var."=".$_REQUEST[$var]."&";
	}
 ...
 $_pagi_enlace_var = $_pagi_query_string;
 ...
 */
 ...
 if ($_pagi_actual != 1){
	...
	//$_pagi_navegacion_temporal[] = "<a ".$_pagi_nav_estilo_mod." href='#' onClick=\"ajax_get('zona_contenido','".$_pagi_enlace."','_pagi_pg=".$_pagi_url."')\">".$_pagi_nav_anterior."</a>";
	$_pagi_navegacion_temporal[] = "<a ".$_pagi_nav_estilo_mod." href='#' onClick=\"ajax_get_tooltip('".$_pagi_div."','".$_pagi_enlace."','".$_pagi_enlace_var."_pagi_pg=".$_pagi_url."'); return false;\">".$_pagi_nav_anterior."</a>"; }
 ...
 for ($_pagi_i = $_pagi_nav_desde; $_pagi_i<=$_pagi_nav_hasta; $_pagi_i++){//Desde página 1 hasta última página ($_pagi_totalPags)
	if ($_pagi_i == $_pagi_actual) {
		...
		//$_pagi_navegacion_temporal[] = "<a ".$_pagi_nav_estilo_mod." href='#' onClick=\"ajax_get('zona_contenido','".$_pagi_enlace."','_pagi_pg=".$_pagi_i."')\">".$_pagi_i."</a>";
		$_pagi_navegacion_temporal[] = "<a ".$_pagi_nav_estilo_mod." href='#' onClick=\"ajax_get_tooltip('".$_pagi_div."','".$_pagi_enlace."','".$_pagi_enlace_var."_pagi_pg=".$_pagi_i."'); return false;\">".$_pagi_i."</a>";		
		//echo "onClick=\"ajax_get('zona_contenido','".$_pagi_enlace."','".$_pagi_enlace_var."pagi_pg=".$_pagi_i."')";
	}
 }
 ...
 if ($_pagi_actual < $_pagi_totalPags){
	...
	//$_pagi_navegacion_temporal[] = "<a ".$_pagi_nav_estilo_mod." href='#' onClick=\"ajax_get('zona_contenido','".$_pagi_enlace."','_pagi_pg=".$_pagi_url."')\">".$_pagi_nav_siguiente."</a>";
	$_pagi_navegacion_temporal[] = "<a ".$_pagi_nav_estilo_mod." href='#' onClick=\"ajax_get_tooltip('".$_pagi_div."','".$_pagi_enlace."','".$_pagi_enlace_var."_pagi_pg=".$_pagi_url."'); return false;\">".$_pagi_nav_siguiente."</a>";
	//echo $_pagi_actual.'-'.$_pagi_totalPags."onClick=\"ajax_get('zona_contenido','".$_pagi_enlace."','".$_pagi_enlace_var."_pagi_pg=".$_pagi_url."')";
 }
...
// if(! = $db->SelectLimit($_pagi_sql, $_pagi_cuantos, $_pagi_inicial)) die(MensajeBase($db->ErrorMsg()));

try {
	//$_pagi_result = $db->SelectLimit("select * from policor_usuario where cod_usuario>=?",10,1,array($key));	
	$_pagi_result = $db->SelectLimit($_pagi_sql, $_pagi_cuantos, $_pagi_inicial, $variables);
	}
	catch  (exception $e) 
	{ 
	print($db->ErrorMsg());
