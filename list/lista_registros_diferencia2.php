...
while ($i<$_SESSION['cantidadroles'])  {
	...
	if (($_SESSION['suc_ban']==1) and ($_SESSION['rol'.$i]!='LAVADO_DINERO_CONFORMA_TODO')  and ($_SESSION['rol'.$i]!='ROL_LAVADO_DINERO_ADM_SIN_CC') and ($_SESSION['rol'.$i]!='ROL_LAVADO_DINERO_ADM_CASINO')and ($_SESSION['rol'.$i]!='ROL_LAVADO_DINERO_ADM_CONFORMA')) { 
		...
		if(($_SESSION['rol'.$i]!='ROL_LAVADO_DINERO_ADM_SIN_CC') and  ($_SESSION['rol'.$i]!='LAVADO_DINERO_CONFORMA_TODO'))
		{
			try {$rsconta = $db->Execute("select a.total, to_char(a.fecha_valor,'DD/MM/YYYY') as fecha_valor, a.nro_asiento
		...
		{
			if($_SESSION['rol'.$i]=='LAVADO_DINERO_CONFORMA_TODO')
			{
			try {$rsconta = $db->Execute("select a.total, to_char(a.fecha_valor,'DD/MM/YYYY') as fecha_valor, a.nro_asiento
                                              from conta_new.asiento_cabecera a, adm.area c
                                              where a.cod_area=c.cod_area
			...
			else
			{
				try {$rsconta = $db->Execute("select a.total, to_char(a.fecha_valor,'DD/MM/YYYY') as fecha_valor, a.nro_asiento
                                              from conta_new.asiento_cabecera a, adm.area c
                                              where a.cod_area=c.cod_area
												  and c.suc_ban in (20,21,22,23,24,25,26,27,30,31,32,33,51)
												  and a.fecha_valor between to_date('$fdesde','DD/MM/YYYY HH24:MI') and to_date('$fhasta','DD/MM/YYYY HH24:MI')
												  and ( upper(a.concepto) like '%UIF%' or upper(a.concepto) like '%U.I.F.%')
											  order by fecha_valor, total desc");
			}
...
if($_SESSION['rol'.$i]!='LAVADO_DINERO_CONFORMA_TODO')
		{
			...
			{
				try {$rsgana = $db->Execute("select valor_premio, to_char(fecha_alta,'dd/mm/yyyy') fecha, id_ganador, conformado, apellido, nombre
			...
			else
			{
				try {$rsgana = $db->Execute("select valor_premio, to_char(fecha_alta,'dd/mm/yyyy') fecha, id_ganador, conformado, apellido, nombre
															  from PLA_AUDITORIA.t_ganador 
															  where suc_ban  in (20,21,22,23,24,25,26,27,30,31,32,33,51)
															  AND FECHA_BAJA IS NULL
																AND USUARIO_BAJA IS NULL
															   and fecha_alta between to_date('$fdesde','DD/MM/YYYY HH24:MI') and to_date('$fhasta','DD/MM/YYYY HH24:MI')
															  order by fecha_alta, valor_premio desc");
...
$pdf->Cell(60,7,'Registros de Contabilidad',1,1,'C');
...
while ($row = $rsconta->FetchNextObject($toupper=true)) {
 $pdf->SetX(60);
	...
	$pdf->Cell(20,7,'$ '.number_format($row->TOTAL,2,',','.'),1,1,'R');			
	$debe+= $row->TOTAL;
	} 
...
$pdf->setx(40);
$pdf->Cell(80,7,'TOTAL $ '.number_format($debe,2,',','.'),0,1,'R');

 $pdf->SetX(60);
$pdf->Cell(95,7,'Registros de Ganadores',1,1,'C');
$pdf->SetFont('Arial','B',9);
 $pdf->SetX(60);
$pdf->Cell(20,8,'Fecha',1,0,'C');
...
$pdf->Cell(180,7,' ',0,1,'R');
$pdf->setx(40);
$pdf->Cell(80,7,'TOTAL $ '.number_format($haber,2,',','.'),0,1,'R');
